      ...
      <section className="min-w-0 border-r border-slate-100">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row">
          <label className="flex h-10 flex-1 items-center gap-2 rounded-xl border border-slate-200 px-3">
            ...
            <input className="w-full bg-transparent text-sm outline-none" placeholder="요청번호, 여행사, 행사명 검색" value={query} onChange={(event) => setQuery(event.target.value)} />
          </label>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td className="px-4 py-10 text-center text-slate-500" colSpan={5}>
                    ...
                    <br />
                    <button className="mt-3 font-semibold text-rose-500" onClick={loadRequests}>다시 시도</button>
                  </td>
                </tr>
              ) : rows.length === 0 ? (
                <tr>
                  ...
                  >
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <b className="truncate">{row.event}</b>
                        {row.priority === '긴급' && (
                          <span className="shrink-0 rounded bg-rose-100 px-1.5 py-0.5 text-[10px] font-bold text-rose-600">긴급</span>
                        )}
                      </div>
                      <p className="mt-1 truncate text-xs text-slate-400">{row.id} · {row.company}</p>
                    </td>
                    ...
                    <td className="whitespace-nowrap px-3 py-4">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${stateColors[row.status]}`}>{row.status}</span>
                    </td>
                    <td className="pr-3">
