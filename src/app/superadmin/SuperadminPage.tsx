...
export function SuperadminDashboardPage({ onSignOut }: { onSignOut?: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        const [agencyRequests, publicGuides, guideRegistrations, activeAdmins, activeSuperAdmins, auditLogs] = await Promise.all([
          getAgencyRequests(),
